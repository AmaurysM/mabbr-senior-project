...
const baseSymbols = {
  ...
  multiplier2x: {
    ...
    icon: <span className="text-2xl font-bold">2x</span>,
    color: 'text-orange-400',
    value: 2,
  },
  multiplier10x: {
    type: 'multiplier10x' as SymbolType,
    icon: <span className="text-2xl font-bold">10x</span>,
    color: 'text-red-500',
    value: 10,
  },
  empty: {
...
STOCK_SYMBOLS.forEach(stock => {
  stockSymbols[`stock_${stock.symbol}`] = {
    ...
    icon: <span className="text-xl font-bold">{stock.symbol}</span>,
    color: 'text-blue-400',
    value: stock.value
  };
});
...
CASH_AMOUNTS.forEach(amount => {
  cashSymbols[`cash_${amount}`] = {
    ...
    icon: <span className="text-xl font-bold">${amount}</span>,
    color: 'text-green-400',
    value: amount
  };
});
...
TOKEN_AMOUNTS.forEach((amount, index) => {
  ...
  if (amount <= 10) {
    ...
    icon = <div className="flex"><FaCoins className="text-2xl" /><FaCoins className="text-2xl ml-1" /></div>;
  } else if (amount <= 500) {
    icon = <div className="flex flex-col items-center">
      <div className="flex"><FaCoins className="text-xl" /><FaCoins className="text-xl ml-0.5" /></div>
      <div className="flex mt-0.5"><FaCoins className="text-xl" /><FaCoins className="text-xl ml-0.5" /></div>
    </div>;
  } else {
...
const ScratchOffPlayContent = () => {
  ...
  const calculatePrize = async (finalWins: { symbolType: SymbolType, count: number, multiplier: number, cellValues?: number[] }[]) => {
    let tokenPrize = 0;
    let cashPrize = 0;
    let stockPrize = 0;
  ...
  // Helper to clear a ticket from localStorage once it's in the database
  const clearTicketFromLocalStorage = () => {
    if (!session?.user?.id) return;
    ...
    if (savedTicketsStr) {
      try {
        const savedTickets = JSON.parse(savedTicketsStr);
        // Filter out this ticket
        const updatedTickets = savedTickets.filter((t: any) => t.id !== ticketId);
        
        // Save back to localStorage
        localStorage.setItem(`user-${userId}-tickets`, JSON.stringify(updatedTickets));
      } catch (error) {
        console.error('Error clearing ticket from localStorage:', error);
      }
    }
  };
