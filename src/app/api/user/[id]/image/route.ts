...
export async function POST(
  ...
  try {
    ...
    // Process the image upload from the form
    ...
    // Delete the old profile picture if it exists and is stored in S3
    if (currentUser?.image && currentUser.image.includes('mabbr-profile-pics.s3')) {
      const oldImageKey = extractS3KeyFromUrl(currentUser.image);
      ...
      if (oldImageKey) {
        try {
          ...
          console.log(`Successfully deleted old profile picture: ${oldImageKey}`);
        } catch (deleteError) {
    ...
    const key = `profile-pictures/${session.user.id}_${Date.now()}_${image.name}`;
    ...
    const imageUrl = `https://mabbr-profile-pics.s3.${process.env.AWS_REGION || 'us-east-2'}.amazonaws.com/${key}`;
